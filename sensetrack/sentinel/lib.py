...
S1Info = namedtuple(
    "S1Info", ['ORBIT_PASS', 'NODE_TIME', 'RELATIVE_ORBIT',
               ...
               'LEVEL', 'CLASS', 'POLARIZATION', 'ABSOLUTE_ORBIT',
               'DATAID', 'ID'])
"""
...
class S1ManifestParser:
    ...
    def parse_filename_regex(cls, filename: str | Path) -> tuple:
        ...
        pattern = r"(S1[A|B])_(\w{2})_(\w{3})(\w{1})_(\w{1})(\w{1})(\w{2})_(\w{15})_(\w{15})_(\w{6})_(\w{6})_(\w{4})."
        ...
        (PLATFORM, MODE, PRODUCT, RES, LEVEL, CLASS, POLARIZATION,
         STARTTIME, ENDTIME, ABSOLUTE_ORBIT, DATAID, ID) = match_obj.groups()

        start_time = datetime.strptime(STARTTIME, "%Y%m%dT%H%M%S")
        end_time = datetime.strptime(ENDTIME, "%Y%m%dT%H%M%S")

        return (PLATFORM, MODE, PRODUCT, RES, LEVEL, CLASS, POLARIZATION,
                start_time, end_time, ABSOLUTE_ORBIT, DATAID, ID)
    ...
    def create_from_filename(cls, filename: str | Path):
        ...
        (PLATFORM, MODE, PRODUCT, RES, LEVEL, CLASS, POLARIZATION,
         _, _, ABSOLUTE_ORBIT, DATAID, ID) = cls.parse_filename_regex(filename)
        
        return S1Info(ORBIT_PASS, NODE_TIME, RELATIVE_ORBIT,
                      PLATFORM, MODE, PRODUCT, RES, LEVEL,
                      CLASS, POLARIZATION, ABSOLUTE_ORBIT, DATAID, ID)
