"""
Module for image processing and registration using optical flow and phase cross-correlation techniques.

This module provides classes and functions to compute optical flow between images and to perform phase cross-correlation between reference and target images. The classes offer interfaces to use optical flow algorithms from OpenCV and scikit-image, while the functions allow conversion of results into formats such as GeoDataFrame (geopandas) or DataFrame (pandas).
...
class OpenCVOpticalFlow(OTAlgorithm):
    ...
    NOTE: Gunnar Farneback's algorithm.
    ...
    def __call__(self, reference: Image, target: Image) -> Image:
        ...
        Arguments:
            ...
            target (Image): The target image to which the optical flow is
                computed.
        Returns:
            Image: An Image object containing the displacement field
                representing the optical flow from the reference to the target image.
        Raises:
            ValueError: If the reference and target images do not have the same
                dimensions.
        """
...
class SkiOpticalFlowILK(OTAlgorithm):
    """
    Compute the optical flow between two images using the ILK (Inverse Lucas-Kanade) method from scikit-image.
    ...
    Returns
        Image: An Image object containing the displacement field representing the optical flow from the reference to the target image.
...
class SkiOpticalFlowTVL1(OTAlgorithm):
    ...
    Methods:
        __call__(reference: Image, target: Image) -> Image
            Computes the optical flow between the reference and target images and returns the displacement field
            as an Image object.
    """
...
class SkiPCC_Vector(OTAlgorithm):
    ...
    Methods
        __call__(reference: Image, target: Image) -> gpd.GeoDataFrame | pd.DataFrame
            Calculates the phase cross-correlation between the reference and target images,
            returning the results as a GeoDataFrame or DataFrame.
    """
