...
class SARPreprocessing(Enum):
    ...
    def validate(cls, name: str) -> Exception | None:
        ...
        graph_path = Path(eval(f"cls.{name.value.upper()}").value)

        if name.value not in members:
            raise ValueError(f"Unrecognized graph name {name.value}.")
...
class SARPreprocessor(ABC):
    ...
    def __init__(self, SUBSET: GPTSubsetter, PROCESS: str) -> None:
        ...
        Raises:
            TypeError: If PROCESS is not a SARPreprocessing enum
